...
const RubricEvaluation = () => {
  ...
  >(undefined);
  const [activeRubricIndex, setActiveRubricIndex] = useState<
    number | undefined
  >(undefined);
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    const containerWidth =
      document.getElementById("rubric-container")?.clientWidth || 0;
  ...
  return (
    <div className={styles.uploadBtnWrapper}>
      ...
      <Modal show={showModal} onHide={handleClose}>
        ...
        <Modal.Footer>
          {isUploading && (
            <button
              type="button"
              className={styles.uploadInProgressButton}
              disabled
            >
              Uploading...
            </button>
      ...
      <div className={styles.rubricWrapper}>
        <div
          id="rubric-container"
          className={styles.rubricContainer}
          style={{ width: `${splitterPosition}%` }}
        >
          <Select
            options={[{ value: "", label: "All Criteria" }, ...options]}
            onChange={(option) =>
              setSelectedCriteria(option ? option.value : null)
            }
          />
          <table className={styles.table}>
            ...
            <tbody>
              {responses
                .filter(
                  (item) =>
                    !selectedCriteria || item.criterion === selectedCriteria,
                )
                .map((item, index) => (
                  <tr key={index}>
                      ...
                      <div className={styles.chatMessageGpt}>
                        <Answer
                          ...
                          onThoughtProcessClicked={() =>
                            onToggleThoughtProcessTab(index)
                          }
                          onSupportingContentClicked={() =>
                            onToggleSupportingContentTab(index)
                          }
        ...
        <div
          ...
          style={{ width: `calc(100% - ${splitterPosition + 1}%)` }}
        >
          {responses.length > 0 &&
            activeCitation &&
            activeRubricIndex !== undefined &&
            activeAnalysisPanelTab !== undefined && (
              <AnalysisPanel
                className=""
                ...
                onActiveTabChanged={(tab) =>
                  onToggleTab(tab, activeRubricIndex)
                }
