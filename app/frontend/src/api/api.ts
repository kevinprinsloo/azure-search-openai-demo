...
export async function uploadFile_csv(file: File, idToken: string | undefined): Promise<string> {
...
export async function uploadFile_pdf(file: File, idToken: string | undefined): Promise<string> {
    const formData = new FormData();
    formData.append("file", file);

    const response = await fetch(`${BACKEND_URI}/upload_pdf`, {
        method: "POST",
        headers: getHeaders(idToken),
        body: formData
    });
